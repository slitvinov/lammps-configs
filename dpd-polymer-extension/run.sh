...
function runsingle() {
    ...
    awk ${awk_debug_flags} -v twotypes=1 -v cutoff=3.0 -v Nbeads=${Nbeads} -v Nsolvent=0 -v Npoly=full \
    ...
    ${mpirun} -n ${np} ${lmp} -v Nbeads ${Nbeads} -v dpdrandom ${RANDOM} -in in.solution
}

#runsingle 2 & sleep 1
runsingle 5
#runsingle 25 & sleep 1
#runsingle 50 & sleep 1
